...
describe('CheckRulesUseCase', () => {
  ...
  it('should return false for invalid regex', async () => {
    const mockRepo = new MockRuleRepository();
    mockRepo.setRules([
      { id: '1', name: 'Test', field: 'body', pattern: '[invalid', isRegex: true, isActive: true }
    ]);
    const useCase = new CheckRulesUseCase(mockRepo);

    const result = await useCase.execute({ sender: '123', body: 'test' });
    expect(result).toBe(false);
  });

  it('should match regex correctly', async () => {
    const mockRepo = new MockRuleRepository();
    mockRepo.setRules([
      { id: '1', name: 'Test', field: 'body', pattern: 'test.*', isRegex: true, isActive: true }
    ]);
    const useCase = new CheckRulesUseCase(mockRepo);

    const result = await useCase.execute({ sender: '123', body: 'testing 123' });
    expect(result).toBe(true);
  });
});